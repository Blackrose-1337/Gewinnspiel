...
import type { Answer, Respond, Survey } from "@/stores/interfaces";
...
export const useCustomerSurveyAnswerStore = defineStore({
    id: "surveyAnswer",
    state: () =>
        ({
            survey: "",
    ...
    actions: {
        ...
        async finish(idsurvey: string, responds: Respond[]) {
            const value = {
                surveyID: idsurvey,
                responds: responds,
            };
            await api.post("answer/surveyanswer", value);
        },
